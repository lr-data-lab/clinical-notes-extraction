...
- **dose**: The amount of drug expressed in mass orvolume. Annotate exactly as written. Its meaning depends on quantity: when quantity is null, dose is the amount administered at one time (Amiodarone 100 mg PO DAILY → 100); when quantity is populated, dose is the strength contained in each unit, and the amount administered is the product of the two (Fluticasone Propionate 110mcg 2 PUFF → 110mcg per puff, 220 mcg administered). Never include count units, units, route, or frequency. Null if no mass, volume, or concentration is stated (Albuterol Inhaler 2 PUFF IH Q6H:PRN sob → null).
